...
Theme cards (required — docs/superpowers/specs/2026-08-06-module-source-depth-design.md): themeCards[] of 3–5 is the ONLY full narrative of each core story (fact → mechanism → trigger → invalidator → horizon → status → grade → optional assets/factSources). One event → one theme card. Skim/summary may list short titles that match theme titles (UI anchors to #theme-{id}); Global/China stay spine facts (Bloomberg). Website does NOT render signals[] — set signals: [] on new briefings (do not invent a parallel Signals/Cross-checks section).

Event Calendar (required — docs/superpowers/specs/2026-08-06-event-calendar-design.md; fixtures: docs/superpowers/specs/2026-08-07-calendar-gov-earnings-design.md): eventCalendar with windowStart=briefing date and windowEnd=next Friday after the Friday-on-or-after that date (Beijing; e.g. Thu Aug 6 → Aug 14; Mon Aug 10 → Aug 21 — always this week + next). Keep ~8–20 vital dated rows (compact) when fixtures exist. Read committed web/content/calendar/gov-fixtures.json and web/content/calendar/earnings-fixtures.json. Coverage spine when in-window: US/China/Japan data+CB+key tech/AI earnings+IPOs; US TIC + quarterly refunding; UK/EU = central-bank only (no UK/EU economic data). China = mainland only — do NOT list Taiwan or Hong Kong prints in eventCalendar (they may still appear in chinaChanged prose). Prefer IMAP 日程/央行动态 then fixtures for gaps through windowEnd. Earnings only from fixtures/watchlist — never invent dates/consensus. Optional themeId chip only (no why/trigger/invalidator). Do NOT emit narrative watchItems for new briefings (legacy field may stay empty array if schema requires).
