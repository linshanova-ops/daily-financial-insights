...
Event Calendar (required — docs/superpowers/specs/2026-08-06-event-calendar-design.md): eventCalendar with windowStart=briefing date and windowEnd=Friday on/after that date (Beijing); ~8–20 dated rows. Coverage spine when in-window: US/China/Japan data+CB+key tech/AI earnings+IPOs; US TIC + quarterly refunding; UK/EU data+CB. Prefer IMAP 日程/央行动态 then standing fixtures — never invent dates/consensus. Optional themeId chip only (no why/trigger/invalidator). Do NOT emit narrative watchItems for new briefings (legacy field may stay empty array if schema requires).
