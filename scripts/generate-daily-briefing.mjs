...
Steps:
1) Merge IMAP inbox on this branch (already fetched). Section map — 国际要闻→globalChanged (1 Chinese bullet each); 大中华新闻→chinaChanged (1 Chinese bullet each); 市场一览→marketOverview.items; 日程+央行动态→watchItems; 今日图表→figures id=bloomberg-chart-of-day kind=insight (open chartImage first). Glassnode Insights (weekly, when captured): use email body only (never fetch "Read full report"); add 1 sourced globalChanged on-chain line + signals/watch when clearly supported; optional figures[] id=glassnode-weekly. Cite stable Bloomberg Asia href only for newsletter; Glassnode newsletter landing for weekly. Attach stronger primaries when available.
