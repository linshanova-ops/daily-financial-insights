...
09:00 is one publish from every source together: 财经早茶 (in Gmail since ~07:00; wait/pull until bloomberg-$TODAY.md exists — GH inbox-sync is this same 09:00) + 见闻 + Caixin/Yicai + BlockBeats + CICC + CNBC/AP closes. Do not write YAML from 见闻 市场收报 while the mail is missing and patch later. $TODAY.md on main is not done unless 今日图表 / 市场一览 / 国际要闻 / 大中华 / 日程 / Themes match that mail and the other desks are in the same YAML. If this leftover wrote a 见闻-only tape, patch the same file. If another agent is RUNNING, that session publishes — do not Retry / do not create a second agent.
No IMAP in this VM; do not `gh workflow run` (403). One pass: 今日图表 (open the PNG), 市场一览, 国际要闻, 大中华, 日程 → calendar (FOMC 2pm ET = 02:00 Beijing next day), then rewrite Themes to the skill’s Theme card block: one force per card, 2–4 Fact lines (`Cite: statement`) that the So what actually uses, So what 2–4 sentences of judgment for the named books — the middle sentence is one dated desk view `House (d Month): view` (Goldman Sachs / Morgan Stanley / JPMorgan / CICC / Glassnode; search CICC per force, 财经早茶 house lines, 见闻 sell-side) only if it adds a mechanism, number, or position the prints lack; none is fine, never pad — ending with the dated next print, grade the tape (not all STRONG), ≤4 chips, desk-view source in keySources, no trigger/invalidator/horizon/status. Prints the So what does not use go to What changed, not the card. Extra FACT prints follow mail bullets. Never invent 市场一览 from Yahoo. After live $TODAY with Themes matching that mail, stop/archive so tomorrow’s 09:00 can fire.
