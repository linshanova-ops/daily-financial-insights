...
No IMAP in this VM; do not `gh workflow run` (403). One pass: 今日图表 (open the PNG), 市场一览, 国际要闻, 大中华, 日程 → calendar (FOMC 2pm ET = 02:00 Beijing next day), then rewrite Themes to the skill’s Theme card block: one force per card, 2–4 Fact lines (`Cite: statement`) that the So what actually uses, So what 2–4 sentences of judgment for the named books — the middle sentence is one dated desk view `House (d Month): view` (CICC search per force, 财经早茶 house lines, 见闻 sell-side, Glassnode, central-bank officials) only if it adds a mechanism, number, or position the prints lack; none is fine, never pad — ending with the dated next print, grade the tape (not all STRONG), ≤4 chips, desk-view source in keySources, no trigger/invalidator/horizon/status. Prints the So what does not use go to What changed, not the card. Extra FACT prints follow mail bullets. Never invent 市场一览 from Yahoo. After live $TODAY with Themes matching that mail, stop/archive so tomorrow’s 09:00 can fire.
