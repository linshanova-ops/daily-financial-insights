...
**Concurrent cap:** Cursor allows one RUNNING cloud agent. Cron dies in &lt;1 min (`rate-limited due to too many concurrent runs`) if a mobile/desktop chat is still RUNNING. **Keep this dashboard automation ON** — it is the 09:00 clock. Archive leftover agents **before** 09:00 Beijing. If a chat is still open after 09:00, that session publishes (patch the same `$TODAY.md` — do not open another agent). After live confirm, **stop**. Do not add a second Cursor cron. GH `inbox-sync.yml` fetches IMAP at this **same 09:00**. `missed-briefing-catchup.yml` at 09:30 may create only if `$TODAY.md` is absent.
...
09:00 is one publish from every source together: 财经早茶 (in Gmail since ~07:00; one 180×30s wait/pull for the same 09:00 inbox-sync; if still missing, bump `# last-kick:` in that workflow file and push — that on.push is the same job, not a second slot — then a short pull; if still missing, name IMAP miss and publish) + 见闻 + Caixin/Yicai + BlockBeats + CICC + CNBC/AP closes. Do not write YAML from 见闻 市场收报 while the mail is missing and patch later. $TODAY.md on main is not done unless 今日图表 / 市场一览 / 国际要闻 / 大中华 / 日程 / Themes match that mail and the other desks are in the same YAML. If this leftover wrote a 见闻-only tape, patch the same file. If another agent is RUNNING, that session publishes — do not Retry / do not create a second agent.
No IMAP in this VM; do not `gh workflow run` (403). One pass: 今日图表 (open the PNG), 市场一览, 国际要闻, 大中华, 日程 → calendar (FOMC 2pm ET = 02:00 Beijing next day), then rewrite Themes to the skill’s Theme card block: one force per card, 2–4 Fact lines (`Cite: statement`) whose numbers the So what uses — unused prints go to What changed — no Yahoo `finance.yahoo.com/quote/` as a theme, cite, or chip (inject stays in marketDashboard), So what 2–4 sentences of judgment the named books can act on until the settle — at most one dated desk view `House (d Month): view` (Goldman Sachs / Morgan Stanley / JPMorgan / CICC / Glassnode; search CICC per force, 财经早茶 house lines, 见闻 sell-side) only if it adds a mechanism, number, or position the prints lack; none is fine, never pad, never clone the same House (date) onto a second card — last sentence is the dated print that settles THIS card’s books, grade the tape (not all STRONG), ≤4 chips, desk-view source in keySources, FACT vs CLAIM (never write CLAIM on the page), no trigger/invalidator/horizon/status. Extra FACT prints follow mail bullets. Never invent 市场一览 from Yahoo. After live $TODAY with Themes matching that mail, stop/archive so tomorrow’s 09:00 can fire.
