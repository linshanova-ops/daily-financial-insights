...
git pull so GH inbox-sync (09:00 Beijing, same minute) Bloomberg/Glassnode captures are on disk. Wait/pull if bloomberg-$TODAY.md or bloomberg-$TODAY.png is missing — empty 今日图表 body is not “no chart” when the PNG exists. No IMAP in this VM; do not `gh workflow run` (403). Merge latest bloomberg-markets-daily-china on or before $TODAY in one pass: 今日图表 (open the PNG), 市场一览, 国际要闻, 大中华, 日程 → calendar (FOMC 2pm ET = 02:00 Beijing next day), then rewrite Themes from that same mail in complete sentences. Never invent 市场一览 from Yahoo. After live $TODAY with Themes matching that mail, stop/archive so tomorrow’s 09:00 can fire.
