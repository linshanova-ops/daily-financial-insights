...
Preflight: git fetch + pull origin main. 财经早茶 is in Gmail ~07:00–07:40; inbox-sync fetches at 08:00 Beijing. Wait/pull until bloomberg-$TODAY.md exists (up to 20 min) — do not write YAML from 见闻 市场收报 or yesterday’s mail. $TODAY.md on main is not done unless 今日图表 / 市场一览 / 国际要闻 / 大中华 / 日程 / Themes already match that file. If this leftover wrote the wrong tape, patch the same file. If another agent is RUNNING, that session publishes — do not Retry / do not create a second agent.
No IMAP in this VM; do not `gh workflow run` (403). One pass from $TODAY 财经早茶: 今日图表 (open the PNG), 市场一览, 国际要闻, 大中华, 日程 → calendar (FOMC 2pm ET = 02:00 Beijing next day), then rewrite Themes from that same mail in complete sentences. Never invent 市场一览 from Yahoo. After live $TODAY with Themes matching that mail, stop/archive so tomorrow’s 09:00 can fire.
