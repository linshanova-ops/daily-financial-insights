...
**Preflight (before gather):** `git fetch origin main`. If `web/content/briefings/$TODAY.md` is already on `origin/main`, stop. Concurrent cap is 1 — a leftover RUNNING desktop/chat agent **kills** `Agent.create` (`rate-limited due to too many concurrent runs`). If this run is still open after 09:00 and `$TODAY.md` is missing, **this session publishes**. After live confirm, **stop/archive** so tomorrow’s 09:00 can `create`. GH `inbox-sync.yml` is the clock: IMAP then nudge at 09:00 Beijing. If that misses, `missed-briefing-catchup.yml` (09:30) **sends** the weekday prompt to the leftover occupying the cap.
