...
| Market closes | `marketDashboard` via inject only. Renders after Summary, **before** Books and Themes. |
...
| Chart | `figures` (chart-of-day **only** if `bloomberg-$TODAY` PNG exists). Renders after Books, before Themes. |
