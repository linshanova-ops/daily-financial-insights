...
| Themes | `themeCards` — **one card per independent market force** from **today’s** 今日图表 + 市场一览 + 国际要闻/大中华 (plus FACT prints). Count follows the tape (quiet day can be 2; a busy 市场一览 can be 6+). Not a news digest, not a 3–5 quota, and not yesterday’s cards with one fact line patched. Title names the books it hits. Required: `assets[]`, `fact`, `factSources`, `mechanism`, `trigger`, `invalidator`, `horizon`, `status`. Site layout: Fact then So what, **full width, stacked** (not two columns); chips = `factSources` under Fact only. Write `fact` / `mechanism` as **period-separated complete sentences**. Each Fact sentence is `Cite: a full statement` on **one line** (cite ≤48 chars, then `: `, then who/what/number — never a bare tape such as `10-year 4.70%` or `Shanghai −0.59%`). The site renders that as `Cite：statement` on the same line. So what is judgment in complete sentences; CICC as `CICC CLAIM (date): …`. Do not paste URLs or a chip list into the sentence. |
