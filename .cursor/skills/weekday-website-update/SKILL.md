...
**Preflight (before gather):** `git fetch origin main`. If `web/content/briefings/$TODAY.md` is already on `origin/main`, stop. Concurrent cap is 1 — a leftover RUNNING desktop/chat agent **kills** the 09:00 Cursor automation (`rate-limited due to too many concurrent runs`). If this run is still open after 09:00 and `$TODAY.md` is missing, **this session publishes**. After live confirm, **stop/archive** so tomorrow’s 09:00 can fire. GH `inbox-sync.yml` fetches IMAP at 09:00 (same minute) and **sends** only if a leftover holds the cap — it does not `Agent.create`. If 09:00 missed, `missed-briefing-catchup.yml` (09:30) sends or creates.
