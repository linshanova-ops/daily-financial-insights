...
| Market closes | `marketDashboard` via inject only. Renders after Summary, **before** 市场一览 and Themes. |
| 市场一览 | `marketOverview` — **Chinese, one bullet each, in mail order**. Do not replace the 财经早茶 desk with English books. Extra dated prints live in Closes / Themes. Do not copy Yahoo inject levels into this paragraph. Do **not** fill `assetClasses`. |
| Chart | `figures` (chart-of-day **only** if `bloomberg-$TODAY` PNG exists). Renders after 市场一览, before Themes. |
