...
5. **Asset framework** — website `assetClasses` (six books: us-equities · asia-equities · rates · fx · commodities · crypto). Per class: regime (current beta), sourced driver or labeled CLAIM, read (what it means + falsifier). Never “see Market Dashboard” as the whole story; levels live in `marketDashboard`. Themes (`themeCards`) are the 3–5 cross-asset forces from **this briefing’s** 今日图表 + 市场一览 + 国际要闻/大中华 (plus FACT prints). Rewrite titles/facts/mechanisms in the same pass as those sections — do not keep yesterday’s cards because `$TODAY.md` already existed. Skim titles = `themeCards[].title`. Chip `themeId` to those ids. See `interpreting-market-signals/references/asset-framework.md`. Legacy `assetFramework` is archives only.
