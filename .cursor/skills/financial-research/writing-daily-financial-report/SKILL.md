...
5. **Themes** — website `themeCards` are **one card per independent market force** from **this briefing’s** 今日图表 + 市场一览 + 国际要闻/大中华 (plus FACT prints). Count follows the tape, not a 3–5 cap. Merge cards that share a mechanism; do not drop a large 市场一览 move to hit a quota. Rewrite titles/facts/mechanisms in the same pass as those sections — do not keep yesterday’s cards because `$TODAY.md` already existed. Homepage skim is `summary` / `signal` / `watch`, not a theme TOC. Chip `themeId` on calendar rows. The site sorts STRONG → MODERATE → WEAK; YAML can stay tape order. The site does **not** render `assetClasses` / `assetFramework` — skip filling six books. Mail **市场一览** fills `marketOverview`; the page title is English **Markets at a glance**. Site stacks Fact above So what, full width. Write each `fact` line as `财经早茶 今日图表：the US Treasury General Account cash is above $900bn.` (cite and statement on one line). One number once — do not restate the same close from a second desk. Never a stacked label or a bare tape; chips only in `factSources`. **Theme So what (`mechanism`)** is 3–5 sentences, no preamble: (1) the pivotal point and core logic of the cited sources, in simple explicit sentences a reader can use — say what the articles mean and why it matters, in words the source would use; do not invent fancy labels (`usage proxy`, `narrative multiple`, `meeting premium`, `profit cushion`, `disruption premium`, and the same pattern); do not restate Fact, Summary, Markets at a glance, or Closes; do not summarize the Fact article again; (2) at most one named external view already in the captured sources (`House (d Month):`), and only when that desk view *is* the logic — never invent a second article, never a second summary; skip if none; (3) the asset effect on this card's `assets` (if it moves no book, say so in one clause; do not invent a spillover); (4) the forward factor that would change the theme (a falsifier, not a calendar pointer, not “the next print is…”). Do not end So what with the dated next print. Fill `trigger`, `invalidator`, `horizon`, and `status` (continuing / escalated / retired / new vs the previous briefing). The date/time of the next print belongs in `eventCalendar` and the skim `watch` string — never in `mechanism`. Bitcoin: `gn` when `GLASSNODE_API_KEY` is set; otherwise the latest Glassnode Insights inbox body, window-dated. Never write the word CLAIM on the website; name the desk. **Global / China:** closes live in `marketDashboard` — do not repeat close prints in Global or China stance / What-changed; keep any line that is not a repeated close.
