...
3. **Global situation** — from `analyzing-global-macro` output, tightened. On the website, `globalRegime` / `globalChanged` / `globalImplies` carry desk news and judgment; **do not repeat index, yield, oil settle, or crypto levels already in Market closes** (`marketDashboard`). Mail-order 国际要闻 bullets stay; do not append a second AP/Treasury close block to What changed.
4. **China situation** — from `analyzing-china-macro` output, tightened. Same contract: `chinaStance` / `chinaChanged` / `chinaImplies` are policy and headlines, not a duplicate A/H close table.
5. **Themes** — website `themeCards` are **one card per independent market force** from **this briefing’s** 今日图表 + 市场一览 + 国际要闻/大中华 (plus FACT prints). Count follows the tape, not a 3–5 cap. Merge cards that share a mechanism; do not drop a large 市场一览 move to hit a quota. Rewrite titles/facts/mechanisms in the same pass as those sections — do not keep yesterday’s cards because `$TODAY.md` already existed. Homepage skim is `summary` / `signal` / `watch`, not a theme TOC. Chip `themeId` on calendar rows. The site sorts STRONG → MODERATE → WEAK; YAML can stay tape order. The site does **not** render `assetClasses` / `assetFramework` — skip filling six books. Mail **市场一览** fills `marketOverview`; the page title is English **Markets at a glance**. Site stacks Fact above So what, full width. Write each `fact` line as `财经早茶 今日图表：the US Treasury General Account cash is above $900bn.` (cite and statement on one line). One number once — do not restate the same close from a second desk. Never a stacked label or a bare tape; chips only in `factSources`. So what (`mechanism`) is 3–5 sentences, no preamble: judgment (the logic; do not restate Fact, Summary, Markets at a glance, or Closes, and do not summarize the Fact article again), exactly one named external view inside that logic as evidence (`House (d Month):`, never a second view and never a second summary), the asset effect on this card's `assets` (if it moves no book, say so in one clause; do not invent a spillover), and the forward factor that would change the theme (a falsifier, not a calendar pointer, not “the next print is…”). Do not end So what with the dated next print. **Plain language:** short sentences; the reader should see the article's pivotal point and why it moves the book — not portfolio slang ("narrative multiple", "discount-rate relief", "meeting premium", etc.). Fill `trigger`, `invalidator`, `horizon`, and `status` (continuing / escalated / retired / new vs the previous briefing). The date/time of the next print belongs in `eventCalendar` and the skim `watch` string — never in `mechanism`. Bitcoin: `gn` when `GLASSNODE_API_KEY` is set; otherwise the latest Glassnode Insights inbox body, window-dated. Never write the word CLAIM on the website; name the desk.
...
8. **Sources & caveats** — website `keySources`: each unique href once, with `books[]` (which asset class it moves) and `influence` (the print or desk view). Inline `factSources`/`driverSources` are the claim’s primary only — do not repeat the whole list. `sources` = leftovers; `singleSource` = caveats.
...
11. **Live market dashboard on every update** — before verify/commit, from `web/` run `node scripts/fetch-market-closes.mjs --inject content/briefings/YYYY-MM-DD.md` so `marketDashboard.asOf` and row prints reflect the latest available closes/intraday (Asia same-day where open; US prior session until US cash close). Set `publishedAt` to current UTC at that same moment (not the first draft time). Note the re-inject timestamp in `singleSource`. **Do not copy inject levels into sourced summary/drivers** that cite `finance.yahoo.com/quote/` — quote HTML has no print; `scan-links` will fail.
