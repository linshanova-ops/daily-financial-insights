...
- **Only using English sources.** Key details often appear in Chinese-language outlets hours earlier; see the sources reference in `gathering-financial-news`. **Required China desks for this pipeline: 华尔街见闻 (Wallstreetcn), Caixin/第一财经 (Yicai), and BlockBeats (律动).** Use them for overnight tape, policy depth, A-share color, and crypto–macro transmission; still verify official prints (PBOC/NBS/CSRC) at primary sources. If tooling can't reach Chinese outlets directly, Xinhua English, Caixin Global, SCMP, and wire China desks are acceptable substitutes — note the limitation rather than silently narrowing coverage.
