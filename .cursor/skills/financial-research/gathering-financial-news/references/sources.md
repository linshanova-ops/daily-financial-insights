...
- **中金点睛 / CICC Research (research.cicc.com)** — **CLAIM desk only** (L2). Use skill `cicc-research-article-search` (`APP_ID`/`APP_SECRET`) after theme keys exist; theme-then-search; include only a dated public-safe paraphrase + link when the note is reachable. Never invent CICC views; never paste VIP full text. Save short paraphrase under `web/content/inbox/cicc/`. If empty → omit and caveat.
...
**syravocado China minimum:** every daily China section should cite at least one coverage-window item each from **华尔街见闻**, **Caixin or 第一财经 (Yicai)**, and **BlockBeats** — or explicitly note the omission in `singleSource` / caveats. **CICC** is optional CLAIM depth, not part of the minimum.
