...
- **BlockBeats / 律动 (theblockbeats.info)** — required China-facing crypto–macro desk. Cover four books when the tape has them (on matching Themes, not four new sections): (1) BTC / ETF / Strategy / stables / exchanges / reg (2) tech/AI chain (3) street ratings and targets (4) macro / CB / fiscal / rates / FX / commodities. Name the desk; never write CLAIM on the page. A flash that only restates a CoinDesk BTC print is not enough. Verify hard macro **and crypto price** prints against tier 1–2. Never publish a BlockBeats BTC/ETH level without confirming the flash's calendar year and an independent dated quote. Skip meme/PnL/KOL price targets.
- **Glassnode** — Week on Chain inbox (`glassnode-insights/`) is a desk view, window-date the email. When `GLASSNODE_API_KEY` is set, `gn metric get` for BTC (see `.cursor/skills/glassnode-cli`). Do not invent on-chain sizes. Product/CLI how-to mail is not a briefing source.
