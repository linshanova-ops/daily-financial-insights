...
6. **China-specific** — policy announcements, regulator statements, property sector, local government debt, major SOE/tech news, capital flows (Stock Connect). Always check **华尔街见闻**, **Caixin / 第一财经 (Yicai)**, and **BlockBeats (律动)** in addition to official sites. 律动 four books when the tape has them (on matching Themes, not new sections): BTC/ETF/MSTR/stables/exchanges/reg; tech/AI chain; street ratings/targets; macro/CB/fiscal/rates/FX/commodities. Skip meme/PnL/KOL targets.
...
- **Verify calendar year, not just month-day.** Aggregator flashes often say "7月15日" without year and can resurface stale 2025 levels in a 2026 sweep. Confirm the page year (or cross-check the level against a dated independent source). Wrong-year items are discarded, not "approximately reused."
- **华尔街见闻 / wallstreetcn.com:** never use a month-day-only A/H or Fed wrap as the sole cite for an index close or speech. Require an explicit year on the page, or confirm the print against a primary/tier-2 same-day source. Article IDs clustered near known-rejected 2025 wraps (e.g. 3751205, 3751275) are a red flag — open the page and check year before logging.
- **Prefer the latest accurate cite.** When several dated sources cover the same fact, keep the newest in-window primary (or best triangulation pair). Never relabel an old URL to a newer date.
- **Prefer primary sources for official data.** Macro prints → BLS / BEA / NBS / central-bank releases; policy rates and speeches → Fed/PBOC/ECB sites; company guidance → company IR. Index closes → official media / exchange reports when available. Secondary outlets are for market color and triangulation, not for inventing the print.
- **Crypto levels need triangulation.** A BlockBeats HTX flash alone is not enough for a published BTC/ETH print — pair with a dated Cointelegraph/CoinDesk/Yahoo (or similar) quote in the coverage window. Glassnode on-chain sizes: latest `glassnode-insights/` body (desk view, window-date the email) or `gn` when `GLASSNODE_API_KEY` is set — never invented. Never write the word CLAIM on the website; name the desk.
...
- **Aggregator JS-only numbers.** BlockBeats and similar pages may embed figures only in client-side JSON. Do not put a hard number in briefing YAML unless it appears in **fetchable page text** at the cited href (what `scan-links` sees in CI). If you saw a level in a rendered excerpt but CI cannot match it, omit the number.
